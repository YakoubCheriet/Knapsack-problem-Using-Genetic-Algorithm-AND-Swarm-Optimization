function Repaired_Chrom = CRO_RatioPW()
...
[~, index] = sort(Vect_Profit ./ Vect_Weight, 'descend'); % Trier Par valeurs
...
for i = 1 : NR_Size % Parcourire les chromes
    ...
    for j = 1 : Objet_Nbr % Parcourire les objets
        ...
        if sum(Get_Chrom_Weight') < Sac_Max_capacity & (rand > 0.4) % Condition
            if Repaired_Chrom (i,index(j)) == 0 % si l'objet n'est pas deja slectionner
                Repaired_Chrom (i,index(j)) = 1; % selectionner l'objet
            end
        else
            if Repaired_Chrom (i,index(j)) == 1 % si l'objet est deja slectionner
                Repaired_Chrom (i,index(j)) = 0; % enlever l'objet
            end
        end
